...
function countUrgent(tasks: Task[], now: Date): number {
  let n = 0;
  for (const task of tasks) {
    if (task.completed || !task.dueDate) continue;
    const group = getDueGroup(task.dueDate, now);
    if (group === 'overdue' || group === 'dueToday' || group === 'dueSoon') n++;
  }
  return n;
}
  ...
  return (
    <div className="relative">
      <button
        ...
        aria-label={
          urgentCount > 0
            ? `${urgentCount} due or overdue tasks`
